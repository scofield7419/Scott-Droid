...
public class MyPagerView extends ViewGroup {
	...
	// 自定义子view的touch事件
	private OnChildCustomTouchListener mChildCustomTouchListener;
	...
	public MyPagerView ( Context context , AttributeSet attrs , int defStyle ) {
		super ( context , attrs , defStyle );
		
		TypedArray a = context.obtainStyledAttributes ( attrs , R.styleable.MyPagerView , defStyle , 0 );
		mCurrentScreen = a.getInt ( R.styleable.MyPagerView_defaultScreen , 0 );
		a.recycle ( );
		init ( );
	}
	...
	private OnTouchListener childTouchListener = new OnTouchListener ( ) {
		public boolean onTouch ( View v , MotionEvent event ) {
			if ( ( v.getId ( ) == R.id.album_area ) && ( event.getAction ( ) == MotionEvent.ACTION_UP ) ) {
				if ( mChildCustomTouchListener != null ) {
					mChildCustomTouchListener.operation ( event );
				}
			}
			return true;
		}
	...
	public boolean onInterceptTouchEvent ( MotionEvent ev ) {
		...
		// 还在滑动，就截止事件不让子view做任何处理
		if ( ( action == MotionEvent.ACTION_MOVE ) && ( mTouchState != TOUCH_STATE_REST ) ) {
			// Toast.makeText ( getContext() ,
			// "starting if (return true):"+mTouchState , 0
			// ).show();
			return true;
		}
		// if ( action == MotionEvent.ACTION_MOVE ) {
		// // Toast.makeText ( getContext() ,
		// "starting if (return true):"+mTouchState , 0 ).show();
		// return true;
		// }
		...
		switch ( action ) {
		 ...
		 // Toast.makeText ( getContext() ,
		 // "ending onInterceptTouchEvent :"+(mTouchState !=
		 // TOUCH_STATE_REST) , 0 ).show();
		return false;
	}
	...
	public boolean onTouchEvent ( MotionEvent ev ) {
		...
		final float x = ev.getX ( );
		// Toast.makeText ( getContext ( ) ,
		// "switch onTouchEvent :"+(action), 0 ).show();
		switch ( action ) {
			case MotionEvent.ACTION_DOWN :
			...
			case MotionEvent.ACTION_MOVE :
				if ( mTouchState == TOUCH_STATE_SCROLLING ) {
					final float t_width = ( getWidth ( ) / 6f );
			...
			case MotionEvent.ACTION_UP :
				if ( mTouchState == TOUCH_STATE_SCROLLING ) {
					...
					velocityTracker.computeCurrentVelocity ( 500 , mMaximumVelocity );// 使用pix/s为单位
		...
		// Toast.makeText ( getContext ( ) ,
		// "ending onTouchEvent :"+(mTouchState != TOUCH_STATE_REST), 0
		// ).show();
	...
	private void snapToDestination ( ) {
		final int screenWidth = getWidth ( );
		// 如果超过屏幕的一半就算是下一个屏
		final int whichScreen = ( getScrollX ( ) + ( screenWidth / 6 ) ) / screenWidth;
	 ...
	 * 注册子view的Touch事件
	 * */
	public void setOnChildCustomTouchListener ( OnChildCustomTouchListener childCustomTouchListener ) {
		if ( childCustomTouchListener != null ) {
			this.mChildCustomTouchListener = childCustomTouchListener;
		}
	}
	 ...
	 * 自定义的一个子view的Touch事件
	 * */
	public interface OnChildCustomTouchListener {
		public void operation ( MotionEvent event );
	}
